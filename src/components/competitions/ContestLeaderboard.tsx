...
import { ArrowLeft, Calendar, ChevronLeft, ChevronRight, Crown, Loader2, Search, Trophy, User, Users, X } from "lucide-react";
...
export default function ContestLeaderboard() {
  const { slug } = useParams({ from: "/_public/competitions/$slug/leaderboard" });
  ...
  } = useContestLeaderboard(contest?.id || "", currentPage, limit);

  const allParticipants = React.useMemo(() => leaderboardData?.data || [], [leaderboardData?.data]);
  ...
  const participants = React.useMemo(() => {
    ...
    return allParticipants.filter((participant) => {
      ...
      const displayName = participant.displayUsername?.toLowerCase() || '';
      const bio = participant.bio?.toLowerCase() || '';
  ...
  const getRankBadge = (rank: number) => {
    switch (rank) {
      case 1:
        return <Badge className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white border-0 shadow-sm font-semibold px-3 py-1">🥇 1st Place</Badge>;
      case 2:
        return <Badge className="bg-gradient-to-r from-gray-400 to-gray-500 text-white border-0 shadow-sm font-semibold px-3 py-1">🥈 2nd Place</Badge>;
      case 3:
        return <Badge className="bg-gradient-to-r from-orange-500 to-orange-600 text-white border-0 shadow-sm font-semibold px-3 py-1">🥉 3rd Place</Badge>;
      default:
        return <Badge variant="outline" className="font-semibold px-3 py-1">#{rank}</Badge>;
    }
  };
  ...
  const openLightbox = (participant: any) => {
  ...
  const getAllImages = (participant: any) => {
    const images: Array<{ id: string; key: string; caption?: string; url: string }> = [];

    // Add cover image if exists
    if (participant.coverImage) {
      images.push({
        id: participant.coverImage,
        key: participant.coverImage,
        url: participant.coverImage,
        caption: undefined,
      });
    }
                    ...
                    <div className="hidden md:block overflow-x-auto rounded-lg border border-border/50">
                      <table className="w-full">
                        <thead className="bg-muted/30">
                          <tr>
                            <th className="text-left py-3 px-4 font-semibold text-foreground text-xs uppercase tracking-wider">Rank</th>
                            ...
                            <th className="text-left py-3 px-4 font-semibold text-foreground text-xs uppercase tracking-wider">Total Votes</th>
                            <th className="text-left py-3 px-4 font-semibold text-foreground text-xs uppercase tracking-wider">Free Votes</th>
                            <th className="text-left py-3 px-4 font-semibold text-foreground text-xs uppercase tracking-wider">Paid Votes</th>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="max-w-7xl mx-auto space-y-4 p-4">
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  <Button variant="outline" size="sm" onClick={() => refetchLeaderboard()} disabled={isLoadingLeaderboard} className="text-xs px-3 py-1">
                    {isLoadingLeaderboard ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <Trophy className="w-3 h-3 mr-1" />}
                    Refresh Leaderboard
                  </Button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-2 flex-1">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center space-x-2">
                        <Trophy className="w-4 h-4 text-primary" />
                        <span className="text-sm font-semibold text-muted-foreground">Leaderboard</span>
                        {isLeaderboardFetching && !searchQuery && (
                        ...
                        {isLoadingLeaderboard && searchQuery && (
                        ...
                        <tbody className="divide-y divide-border/50">
                          {participants.map((participant, index) => (
                            <tr
                              ...
                              onMouseEnter={() => setHoveredParticipant(participant.profileId)}
                            ...
                            >
                                ...
                                <div className="flex items-center space-x-3">
                                  <div className="relative">
                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center ring-1 ring-primary/20 group-hover:ring-primary/40 transition-all duration-300">
                                      {participant.avatarUrl ? (
                                        (() => {
                                          const optimizedAvatarImage = ImageHelper.avatar(participant.avatarUrl, "thumbnail");
                                          return <img src={optimizedAvatarImage} alt={participant.username} className="w-8 h-8 rounded-full object-cover" />;
                                        })()
                                      ) : (
                                  ...
                                  <div className="space-y-0.5">
                                    <p className="font-semibold text-foreground text-sm group-hover:text-primary transition-colors duration-200">
                                      {highlightSearchTerm(participant.displayUsername || participant.username || "Unknown User", debouncedSearch)}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      @{highlightSearchTerm(participant.username || "", debouncedSearch)}
                                    </p>
                                  </div>
                                </div>
                              </td>
                              <td className="py-3 px-4">
                                <div className="flex items-center space-x-2">
                                  ...
                                  {participant.coverImage ? (
                                    (() => {
                                      const optimizedCoverImage = ImageHelper.cover(participant.coverImage, "small");
                                      return (
                              ...
                              <td className="py-3 px-4">
                                <div className="text-center">
                                  <p className="font-bold text-lg text-primary">{participant.totalVotes}</p>
                                </div>
                              </td>
                              <td className="py-3 px-4">
                                <div className="text-center">
                                  <p className="text-sm text-green-600 font-semibold">{participant.freeVotes}</p>
                                </div>
                              </td>
                              <td className="py-3 px-4">
                                <div className="text-center">
                                  <p className="text-sm text-blue-600 font-semibold">{participant.paidVotes}</p>
                                </div>
                              </td>
                              <td className="py-3 px-4">
                                <div className="flex items-center space-x-2">
                                  <Link to="/profile/$username" params={{ username: participant.username || "" }}>
                                    <Button
                                      variant="outline"
                                      size="sm"
                                    ...
                                    >
                                      <User className="w-3 h-3 mr-1 group-hover/btn:scale-110 transition-transform duration-200" />
                                      View Profile
                                    </Button>
                                  </Link>
                                </div>
                          ...
                          <div className="flex flex-col items-start gap-3">
                            <div className="flex items-start space-x-3 w-full">
                              <div className="relative">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center ring-1 ring-primary/20 group-hover:ring-primary/40 transition-all duration-300">
                                  {participant.avatarUrl ? (
                                    <img src={participant.avatarUrl} alt={participant.username} className="w-10 h-10 rounded-full object-cover" />
                                  ) : (
                            ...
                            {participant.coverImage && (
                              <div
                                ...
                                onClick={() => openLightbox(participant)}
                              >
                                <img src={participant.coverImage} alt="Cover" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 rounded-lg flex items-center justify-center">
                            ...
                            <div className="grid grid-cols-3 gap-2 w-full">
                              <div className="text-center p-2 bg-primary/5 rounded-lg">
                                ...
                                <p className="font-bold text-primary">{participant.totalVotes}</p>
                              </div>
                              <div className="text-center p-2 bg-green-50 rounded-lg">
                                <p className="text-xs text-muted-foreground">Free</p>
                                <p className="font-bold text-green-600">{participant.freeVotes}</p>
                              </div>
                              <div className="text-center p-2 bg-blue-50 rounded-lg">
                                <p className="text-xs text-muted-foreground">Paid</p>
                                <p className="font-bold text-blue-600">{participant.paidVotes}</p>
                              </div>
                            </div>

                            <div className="flex items-center justify-between w-full pt-1 border-t border-border/30">
                              <div className="text-xs text-muted-foreground">Rank #{participant.rank}</div>
                              <Link to="/profile/$username" params={{ username: participant.username || "" }}>
                                <Button
                                  variant="outline"
                                  size="sm"
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="text-center text-white space-y-3">
                      <div className="flex items-center justify-center space-x-3">
                        ...
                        <p className="text-xl font-semibold">{selectedParticipant.displayUsername || selectedParticipant.username || "Unknown User"}</p>
                      </div>
